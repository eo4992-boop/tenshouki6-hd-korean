...
6. 프로그램이 메모리를 검색한 뒤 후보 주소 전체를 4개씩 그룹으로 나누고, 약 0.75초마다 하드웨어 데이터 브레이크포인트 그룹을 자동 순환합니다.
...
```
=== NOBU6HD STRING TRACE V2.2 START ===
PID=1234
TARGET="織田信長"
FOUND=2
...
- 32비트(Win32) 빌드입니다.
- x86 하드웨어 데이터 브레이크포인트는 동시에 최대 4개이므로, V2.2는 모든 후보를 4개씩 순환 추적합니다.
- `[STRING ACCESS]`가 나오면 target 주소, READ/WRITE 종류, EIP, 모듈/RVA, 해당 EIP의 명령 바이트를 함께 기록합니다.
